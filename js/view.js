	...
	return {
		...
		link: function(scope, element, attrs) {
			...
			function createWaves() {
				...
				for(var quantity2 = 0, len2 = totalPoints; quantity2 < len2; quantity2++)
						
					// Second wave
					wave2.push({
						
						x: canvas.width * quantity2 / (len2 - 1),
						y: canvas.height * 0.3,
						vy: Math.random() * 10,
						
						depth: canvas.height * 0.3
						
					});
					
				for(var quantity3 = 0, len3 = totalPoints; quantity3 < len3; quantity3++)
						
					// Third wave
					wave3.push({
						
						x: canvas.width * quantity3 / (len3 - 1),
						y: canvas.height * 0.3 + 20,
						vy: Math.random() * 10,
						
						depth: canvas.height * 0.4
			...
			function render() {
				...
				for(var wave = 0; wave < (wave1.length || wave2.length || wave3.length); wave++) {  
					...
					context.save();
					context.globalAlpha = 0.5;
					context.fillStyle = '#86122a';
					context.beginPath();
					
					context.moveTo(wave2[0].x, wave2[0].y);
					
					// Draw through N wave2
					for(var N2 = 1; N2 < wave2.length - 2; N2++) {
					
						var xc2 = (wave2[N2].x + wave2[N2 + 1].x) / 2;
						var yc2 = (wave2[N2].y + wave2[N2 + 1].y) / 2;
					
						context.quadraticCurveTo(wave2[N2].x, wave2[N2].y, xc2, yc2);
					...
					context.save();
					context.globalAlpha = 0.5;
					context.fillStyle = '#b41738';
					context.beginPath();
					
					context.moveTo(wave3[0].x, wave3[0].y);
					
					// Draw through N wave3
					for(var N3 = 1; N3 < wave3.length - 2; N3++) {
					
						var xc3 = (wave3[N3].x + wave3[N3 + 1].x) / 2;
						var yc3 = (wave3[N3].y + wave3[N3 + 1].y) / 2;
					
						context.quadraticCurveTo(wave3[N3].x, wave3[N3].y, xc3, yc3);
